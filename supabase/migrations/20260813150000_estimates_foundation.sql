...
create index if not exists estimates_customer_date_idx
  on public.estimates (customer_id, issue_date desc);
...
create policy backend_only on public.estimates
  for all to anon, authenticated
  using (false) with check (false);

drop policy if exists backend_only on public.estimate_items;
create policy backend_only on public.estimate_items
  for all to anon, authenticated
  using (false) with check (false);
