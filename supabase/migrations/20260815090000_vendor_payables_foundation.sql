-- Phase 9: vendor payables and purchase accounting foundation.
...
create index if not exists vendor_payments_payment_date_idx
  on public.vendor_payments(payment_date);
create index if not exists vendor_payment_allocations_payment_id_idx
  on public.vendor_payment_allocations(payment_id);
create index if not exists vendor_payment_allocations_purchase_id_idx
  on public.vendor_payment_allocations(purchase_id);
...
begin
  ...
  if not found then
    raise exception 'Allocations must reference purchases for the selected vendor' using errcode = '23503';
  end if;

  -- Existing idempotency key: return the original payment only when its fingerprint matches.
