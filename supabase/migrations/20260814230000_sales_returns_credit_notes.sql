...
-- A credit note is an authoritative reversing sales transaction.
-- It restores inventory, reverses COGS, and credits the customer's receivable atomically.
...
declare
  ...
  v_result jsonb;
begin
  if btrim(p_credit_note_number) = '' or btrim(p_reason) = '' then
  ...
  from public.credit_note_idempotency_keys
  ...
  if found then
    ...
    from public.credit_notes cn where cn.id = v_existing.credit_note_id;
    return v_result;
  end if;
  ...
  for update;
  if not found then
    raise exception using errcode = '23503', message = 'Invoice does not belong to the specified customer';
  end if;
  if v_invoice_item.status = 'VOID' then
    raise exception using errcode = '22023', message = 'Cannot return items from a void invoice';
  end if;

  insert into public.credit_notes (
    credit_note_number, invoice_id, customer_id, credit_date, currency_code, status, subtotal, grand_total, reason, notes
  ) values (
    p_credit_note_number, p_invoice_id, p_customer_id, p_credit_date, upper(btrim(p_currency_code)), 'POSTED', 0, 0, btrim(p_reason), nullif(btrim(p_notes), '')
  ) returning id into v_credit_note_id;

  for v_item in select * from jsonb_array_elements(p_items) loop
    select ii.* into v_invoice_item
    from public.invoice_items ii
    where ii.id = (v_item->>'invoice_item_id')::bigint
      and ii.invoice_id = p_invoice_id
    for update;
    if not found then
      raise exception using errcode = '23503', message = 'Invoice item does not belong to the invoice';
    end if;
    v_qty := (v_item->>'quantity')::numeric;
    if v_qty <= 0 then
      raise exception using errcode = '22023', message = 'Return quantity must be positive';
    end if;
    if v_invoice_item.unit_cost is null then
    ...
    where ci.invoice_item_id = v_invoice_item.id and cn.status = 'POSTED';

    if v_returned + v_qty > v_invoice_item.quantity then
      raise exception using errcode = 'P0002', message = 'Return quantity exceeds the remaining invoice quantity';
    end if;

    v_line_total := v_qty * v_invoice_item.unit_price;
    v_subtotal := v_subtotal + v_line_total;
    v_cogs := v_cogs + (v_qty * v_invoice_item.unit_cost);

    insert into public.credit_note_items (
      credit_note_id, invoice_item_id, product_id, warehouse_id, quantity, unit, unit_price, line_total, unit_cost, cogs_reversal_total
    ) values (
      v_credit_note_id, v_invoice_item.id, v_invoice_item.product_id, (v_item->>'warehouse_id')::bigint,
      v_qty, v_invoice_item.unit, v_invoice_item.unit_price, v_line_total, v_invoice_item.unit_cost, v_qty * v_invoice_item.unit_cost
    );

    insert into public.stock_movements (
      product_id, warehouse_id, movement_type, quantity, reference_type, reference_id, unit_cost, notes
    ) values (
      v_invoice_item.product_id, (v_item->>'warehouse_id')::bigint, 'SALE_RETURN', v_qty,
      'CREDIT_NOTE', v_credit_note_id, v_invoice_item.unit_cost, 'Sales return / credit note'
    );

    insert into public.inventory (product_id, warehouse_id, quantity)
    values (v_invoice_item.product_id, (v_item->>'warehouse_id')::bigint, v_qty)
    on conflict (product_id, warehouse_id)
    do update set quantity = public.inventory.quantity + excluded.quantity, updated_at = now();
  end loop;

  update public.credit_notes
  set subtotal = v_subtotal, grand_total = v_subtotal, updated_at = now()
  where id = v_credit_note_id;

  insert into public.customer_ledger_entries (
    customer_id, entry_type, reference_type, reference_id, debit, credit, currency_code, entry_date, description
  ) values (
    p_customer_id, 'CREDIT_NOTE', 'CREDIT_NOTE', v_credit_note_id, 0, v_subtotal,
    upper(btrim(p_currency_code)), p_credit_date, 'Sales return credit note'
  );

  insert into public.accounting_journal_entries (entry_date, source_type, source_id, description)
  values (p_credit_date, 'CREDIT_NOTE', v_credit_note_id, 'Sales return / credit note')
  returning id into v_journal_id;

  insert into public.accounting_journal_lines (journal_entry_id, account_code, debit, credit, description)
  values
    (v_journal_id, 'SALES_RETURNS', v_subtotal, 0, 'Sales return reversal'),
    (v_journal_id, 'ACCOUNTS_RECEIVABLE', 0, v_subtotal, 'Customer receivable credit'),
    (v_journal_id, 'INVENTORY', v_cogs, 0, 'Inventory restored'),
    (v_journal_id, 'COGS', 0, v_cogs, 'COGS reversal');

  insert into public.credit_note_idempotency_keys (
    principal_scope, operation, idempotency_key, request_fingerprint, credit_note_id
  ) values (
    p_principal_scope, p_operation, p_idempotency_key, p_request_fingerprint, v_credit_note_id
  );
