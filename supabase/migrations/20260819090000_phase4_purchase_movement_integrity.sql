-- Phase 4 audit closure: enforce purchase stock-movement integrity and remove known legacy test rows.
-- The two rows below were created by historical transaction-engine tests and do not reference real purchases.
...
-- Reconcile the affected inventory balance to the remaining authoritative movement ledger.
...
-- A purchase movement must always point to a real purchase.
...
BEGIN
  IF NEW.movement_type = 'PURCHASE' THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM public.purchases p WHERE p.id = NEW.reference_id
    ) THEN
...
-- Prevent duplicate purchase movements for the same purchase/product/warehouse.
