-- Phase 7A: Customer Payments & Receivables Foundation.
-- Payments are authoritative financial transactions. Allocation, customer ledger,
-- invoice status, and accounting journal posting occur atomically in one function.
...
create table if not exists public.customer_payment_allocations (
  ...
  amount numeric not null check (amount > 0),
  created_at timestamptz not null default now(),
  constraint customer_payment_invoice_unique unique (payment_id, invoice_id)
);

create table if not exists public.customer_payment_idempotency_keys (
  id bigint generated always as identity primary key,
  principal_scope text not null,
  idempotency_key text not null,
  operation text not null,
  request_fingerprint text not null,
  payment_id bigint references public.customer_payments (id),
  created_at timestamptz not null default now(),
  constraint customer_payment_idempotency_unique unique (principal_scope, operation, idempotency_key)
);
...
alter table public.customer_payment_idempotency_keys enable row level security;

revoke all on public.customer_payments from anon, authenticated;
revoke all on public.customer_payment_allocations from anon, authenticated;
revoke all on public.customer_payment_idempotency_keys from anon, authenticated;
