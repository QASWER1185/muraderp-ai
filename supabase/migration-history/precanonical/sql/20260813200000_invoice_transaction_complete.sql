-- MuradERP-AI complete Invoice transaction workflow.
-- Invoice creation/posting is one authoritative transaction; there is no separate Post action.
-- Drafts may be deleted after confirmation. Posted invoices are reversed through VOID, never hard-deleted.
...
create index if not exists sales_transaction_idempotency_invoice_idx
  on public.sales_transaction_idempotency_keys(invoice_id);

alter table public.salespeople enable row level security;
alter table public.invoice_transaction_events enable row level security;
revoke all on public.salespeople from anon, authenticated;
revoke all on public.invoice_transaction_events from anon, authenticated;
...
begin
  ...
  if coalesce((p_invoice->>'customer_id')::bigint,0) <= 0 then
  ...
  insert into public.invoices(
    invoice_number,customer_id,source_estimate_id,source_type,issue_date,currency_code,
    status,subtotal,discount_total,grand_total,pass_through_rent,salesperson_id,warehouse_id,notes
  ) values(
    p_invoice->>'invoice_number',(p_invoice->>'customer_id')::bigint,
    nullif(p_invoice->>'source_estimate_id','')::bigint,coalesce(p_invoice->>'source_type','DIRECT'),
    coalesce((p_invoice->>'issue_date')::date,current_date),coalesce(p_invoice->>'currency_code','PKR'),
    'DRAFT',coalesce((p_invoice->>'subtotal')::numeric,0),coalesce((p_invoice->>'discount_total')::numeric,0),
    coalesce((p_invoice->>'grand_total')::numeric,0),coalesce((p_invoice->>'pass_through_rent')::numeric,0),
    nullif(p_invoice->>'salesperson_id','')::bigint,nullif(p_invoice->>'warehouse_id','')::bigint,p_invoice->>'notes'
  ) returning id into v_invoice_id;

  insert into public.invoice_items(
    invoice_id,line_number,product_id,quantity,unit,unit_price,line_total,unit_cost,cogs_total,pricing_source
  )
  select v_invoice_id,(x->>'line_number')::integer,(x->>'product_id')::bigint,(x->>'quantity')::numeric,
    coalesce(x->>'unit','unit'),(x->>'unit_price')::numeric,(x->>'line_total')::numeric,
    nullif(x->>'unit_cost','')::numeric,nullif(x->>'cogs_total','')::numeric,coalesce(x->>'pricing_source','MANUAL')
...
declare
  ...
  v_fingerprint text := md5(coalesce(p_invoice::text,'') || '|' || coalesce(p_lines::text,'') || '|' || coalesce(p_warehouse_id::text,''));
  ...
  v_journal_id bigint;
begin
  if coalesce(trim(p_principal_id),'')='' or coalesce(trim(p_idempotency_key),'')='' then
    raise exception using errcode='22023', message='principal_id and idempotency_key are required';
  end if;
  ...
  select invoice_id,request_fingerprint into v_existing_invoice,v_existing_fingerprint
  ...
  if v_existing_invoice is not null then
    if v_existing_fingerprint is not null and v_existing_fingerprint<>v_fingerprint then
      raise exception using errcode='23505', message='idempotency key was reused for a different invoice request';
    end if;
    return jsonb_build_object('invoice_id',v_existing_invoice,'replayed',true);
  end if;

  insert into public.invoices(
    invoice_number,customer_id,source_estimate_id,source_type,issue_date,currency_code,status,
    subtotal,discount_total,grand_total,pass_through_rent,salesperson_id,warehouse_id,notes
  ) values(
    p_invoice->>'invoice_number',(p_invoice->>'customer_id')::bigint,
    nullif(p_invoice->>'source_estimate_id','')::bigint,coalesce(p_invoice->>'source_type','DIRECT'),
    coalesce((p_invoice->>'issue_date')::date,current_date),coalesce(p_invoice->>'currency_code','PKR'),
    'POSTED',v_subtotal,v_discount,v_grand_total,v_rent,nullif(p_invoice->>'salesperson_id','')::bigint,
  ...
  for v_item in select value from jsonb_array_elements(p_lines) loop
    ...
    if v_unit_cost is null then
      raise exception using errcode='23514', message='unit cost is required for COGS';
    end if;
    v_cogs:=v_cogs+((v_item->>'quantity')::numeric*v_unit_cost);
    v_revenue:=v_revenue+(v_item->>'line_total')::numeric;

    update public.inventory set quantity=quantity-(v_item->>'quantity')::numeric,updated_at=now() where id=v_inventory.id;
    ...
    values(v_invoice_id,(v_item->>'line_number')::integer,(v_item->>'product_id')::bigint,(v_item->>'quantity')::numeric,
      coalesce(v_item->>'unit','unit'),(v_item->>'unit_price')::numeric,(v_item->>'line_total')::numeric,v_unit_cost,
      (v_item->>'quantity')::numeric*v_unit_cost,coalesce(v_item->>'pricing_source','MANUAL'));
  end loop;

  -- Revenue is product sales only. Pass-through rent is never revenue.
  if v_revenue<>v_grand_total then
    raise exception using errcode='23514', message='invoice line revenue does not equal grand total';
  end if;
  ...
  if v_rent>0 then
    insert into public.accounting_journal_lines(journal_entry_id,account_code,debit,credit,description) values
      (v_journal_id,'RENT_RECEIVABLE',v_rent,0,'Pass-through rent receivable'),
      (v_journal_id,'RENT_PAYABLE',0,v_rent,'Pass-through rent payable');
  end if;

  insert into public.sales_transaction_idempotency_keys(principal_id,idempotency_key,invoice_id,request_fingerprint)
  values(p_principal_id,p_idempotency_key,v_invoice_id,v_fingerprint);
  insert into public.invoice_transaction_events(invoice_id,event_type,principal_id)
  values(v_invoice_id,'POSTED',p_principal_id);

  return jsonb_build_object('invoice_id',v_invoice_id,'replayed',false,'revenue',v_revenue,'cogs',v_cogs,'rent',v_rent,'profit',v_revenue-v_cogs);
exception when unique_violation then
  raise exception using errcode='23505', message='duplicate invoice or idempotency key';
end;
$$;

create or replace function public.delete_invoice_draft(p_invoice_id bigint,p_principal_id text)
returns boolean
language plpgsql
security definer
set search_path=public
...
begin
  ...
  insert into public.invoice_transaction_events(invoice_id,event_type,principal_id) values(p_invoice_id,'DELETED',p_principal_id);
  delete from public.invoices where id=p_invoice_id;
  return true;
end;
$$;

create or replace function public.void_invoice_atomic(p_invoice_id bigint,p_principal_id text)
returns boolean
language plpgsql
security definer
set search_path=public
...
begin
  ...
  insert into public.invoice_transaction_events(invoice_id,event_type,principal_id) values(p_invoice_id,'VOIDED',p_principal_id);
  return true;
end;
$$;

revoke all on function public.create_invoice_draft(jsonb,jsonb,text) from public,anon,authenticated;
revoke all on function public.post_invoice_atomic(jsonb,jsonb,bigint,text,text) from public,anon,authenticated;
revoke all on function public.delete_invoice_draft(bigint,text) from public,anon,authenticated;
revoke all on function public.void_invoice_atomic(bigint,text) from public,anon,authenticated;
