-- MuradERP-AI sales transaction foundation.
-- Estimate remains non-posting. Invoice is the authoritative sales transaction.
-- This migration establishes the persistence boundary; the application transaction
-- service will orchestrate inventory, receivable, revenue, COGS and rent atomically.
...
create table if not exists public.invoice_items (
  ...
  constraint invoice_items_cogs_matches check (
    (unit_cost is null and cogs_total is null)
    or (unit_cost is not null and cogs_total = quantity * unit_cost)
  )
);
