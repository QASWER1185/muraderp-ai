...
describe("Phase 23 Stage 5 — deterministic pricing regression", () => {
  it("uses CUSTOMER > VENDOR > GLOBAL precedence and the newest effective version with the highest quantity tier", async () => {
    ...
    const resolved = await repository.findBestRateListItem({ organization_id: ORGANIZATION_ID, price_type: "SALE", product_id: 500, quantity: 10, as_of: "2026-08-16T12:00:00Z", vendor_id: 20, customer_id: 30 });
  ...
  it("honors an explicitly selected rate list instead of falling back to contextual pricing", async () => {
    const repository = new SupabaseRateListRepository(() => clientFor({ rate_lists: rateLists, rate_list_versions: versions, rate_list_items: items }) as never);
    const resolved = await repository.findBestRateListItem({ organization_id: ORGANIZATION_ID, price_type: "SALE", product_id: 500, quantity: 1, as_of: "2026-08-16T12:00:00Z", vendor_id: 20, customer_id: 30, rate_list_id: 2 });
  ...
  it("rejects ambiguous winning-scope pricing instead of silently choosing one list", async () => {
    ...
    await expect(repository.findBestRateListItem({ organization_id: ORGANIZATION_ID, price_type: "SALE", product_id: 500, quantity: 1, as_of: "2026-08-16T12:00:00Z", customer_id: 30 })).rejects.toThrow("Ambiguous pricing");
  });
  ...
  it("returns null when no deterministic product price exists", async () => {
    ...
    const resolved = await repository.findBestRateListItem({ organization_id: ORGANIZATION_ID, price_type: "SALE", product_id: 999, quantity: 1, as_of: "2026-08-16T12:00:00Z" });
  ...
  it("keeps AI/voice/OCR candidate handling limited to deterministic rate-list context", async () => {
    ...
    const resolved = await service.resolveCandidate({ product_id: 500, quantity: 10, selection_source: "OCR_BRAND_MATCH", selected_rate_list_id: 3 }, { organization_id: ORGANIZATION_ID, price_type: "SALE", as_of: "2026-08-16T12:00:00Z", customer_id: 30, vendor_id: 20 });
