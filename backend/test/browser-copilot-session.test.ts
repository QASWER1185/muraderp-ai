...
describe("Phase 22 secure browser Copilot boundary", () => {
  it("rejects Copilot execution without a browser session or internal token", async () => {
    const app = express(); app.use(express.json()); app.use("/auth", authRouter); app.use("/copilot", createAiCopilotRouter(INTERNAL, { createDraft: vi.fn(), confirmAndExecute: vi.fn() } as any));
  ...
  it("keeps internal confirmation protected and returns execution state", async () => {
    ...
    const app = express(); app.use(express.json()); app.use("/auth", authRouter); app.use("/copilot", createAiCopilotRouter(INTERNAL, runtime as any));
    const response = await request(app).post("/copilot/drafts/00000000-0000-4000-8000-000000000003/confirm").set("Authorization", `Bearer ${INTERNAL}`).set("X-Organization-Id", ORG).set("X-User-Id", USER).set("Idempotency-Key", "browser-boundary-test");
