...
describe("database-backed ERP API", () => {
  ...
  it("requires Idempotency-Key before purchase persistence", async () => {
    ...
    const response = await request(app)
      ...
      .set("Authorization", `Bearer ${internalToken}`)
      .send({ vendor_id: 11, warehouse_id: 21, items: [{ product_id: 31, quantity: 3, unit_cost: 10 }] });
  ...
  it("passes stable principal, key, and deterministic fingerprint to purchase persistence", async () => {
    ...
    const first = await request(app)
      .post("/api/v1/purchases")
      .set("Authorization", `Bearer ${internalToken}`)
      .set("Idempotency-Key", "purchase-key-501")
      .send(input);
    ...
    const second = await request(app)
      ...
      .set("Authorization", `Bearer ${internalToken}`)
      .set("Idempotency-Key", "purchase-key-501")
      .send({ ...input, invoice_number: "test-invoice-501" });
    ...
    expect(recordPurchase.mock.calls[1]![1].requestFingerprint).toBe(
      recordPurchase.mock.calls[0]![1].requestFingerprint,
    );
  });
  ...
  it("validates an atomic purchase before calling the database function", async () => {
    ...
    const response = await request(app)
      .post("/api/v1/purchases")
      .set("Authorization", `Bearer ${internalToken}`)
      .set("Idempotency-Key", "purchase-key-legacy-test")
      .send(input);
    ...
    expect(recordPurchase).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        principalScope: principalId,
        operation: "purchase.create",
        idempotencyKey: "purchase-key-legacy-test",
      }),
    );
  });
  ...
  it("rejects a partial purchase before any persistent call", async () => {
    ...
    const response = await request(app)
      .post("/api/v1/purchases")
      .set("Authorization", `Bearer ${internalToken}`)
      .set("Idempotency-Key", "purchase-validation-test")
