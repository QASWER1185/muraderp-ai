...
/** Invoice creation is the user-facing sales action; its transaction effects are atomic. */
...
export class DefaultInvoiceService {
  ...
  /** Quotation is only a presentation label for the same Estimate document. */
  async createFromQuotation(
    estimate: EstimateDocument,
    definition: InvoiceDefinition,
  ): Promise<InvoiceTransactionResult> {
    return this.createFromEstimate(estimate, definition);
  }
  ...
  async createDirect(
    ...
    return this.transaction.execute(buildDirectInvoice(
      definition,
      lines,
      subtotal,
      discount_total,
