...
export interface CustomerPaymentService {
  recordPayment(
    ...
    idempotency: CustomerPaymentIdempotencyContext,
...
function databaseError(error: { code?: string; message?: string }, operation: string): ApiError {
  switch (error.code) {
    case "P0001":
      return new ApiError(409, "IDEMPOTENCY_KEY_REUSED", "The Idempotency-Key was already used for a different payment request");
    case "P0002":
      return new ApiError(409, "OVERPAYMENT", "Payment allocation exceeds invoice outstanding balance");
    case "23503":
      return new ApiError(409, "REFERENCE_CONFLICT", "A related customer or invoice was not found");
    case "23505":
      return new ApiError(409, "DUPLICATE_RECORD", "A payment allocation already exists");
    case "22023":
      return new ApiError(400, "BUSINESS_RULE_VIOLATION", error.message ?? "The payment violates a business rule");
    case "42501":
...
export class SupabaseCustomerPaymentService implements CustomerPaymentService {
  ...
  async recordPayment(
    ...
    if (!data || typeof data !== "object" || Array.isArray(data)) {
      throw new ApiError(502, "INVALID_DATABASE_RESPONSE", "Customer payment returned an invalid response");
    }

    const result = data as unknown as CustomerPaymentResult;
    if (!result.payment || !Array.isArray(result.allocations)) {
      throw new ApiError(502, "INVALID_DATABASE_RESPONSE", "Customer payment response is incomplete");
    }
