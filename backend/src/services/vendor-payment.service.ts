...
export type VendorPaymentContext = AuthoritativeTransactionContext<"vendor-payment.create">;
...
function mapDatabaseError(error: { code?: string; message?: string }): ApiError {
  switch (error.code) {
    ...
    case "42501": return new ApiError(403, "AUTHORIZATION_DENIED", error.message ?? "The vendor payment is not authorized for this organization or branch");
