...
const scopePriority: Record<RateListScopeType, number> = {
  CUSTOMER: 3,
  VENDOR: 2,
  GLOBAL: 1,
};
...
export class SupabaseRateListRepository implements RateListRepository, RateListLifecycleRepository {
  ...
  async listActiveSaleRateLists(): Promise<RateListRecord[]> {
    ...
    const { data, error } = await client
      .from("rate_lists")
      .select(
        "id, name, code, price_type, scope_type, vendor_id, customer_id, currency_code, is_active, created_at, updated_at",
      )
      .eq("price_type", "SALE")
      .eq("is_active", true)
  ...
  async findBestRateListItem(context: PriceResolutionContext): Promise<ResolvedPrice | null> {
    ...
    let rateListQuery = client
      .from("rate_lists")
      .select(
        "id, name, code, price_type, scope_type, vendor_id, customer_id, currency_code, is_active, created_at, updated_at",
      )
      .eq("price_type", context.price_type)
      .eq("is_active", true);

    if (context.rate_list_id != null) {
      rateListQuery = rateListQuery.eq("id", context.rate_list_id);
    }

    const { data: rawRateLists, error: rateListError } = await rateListQuery;
    if (rateListError) throw rateListError;

    const rateLists = (rawRateLists ?? []) as RateListRecord[];
    const applicableRateLists = rateLists.filter((rateList: RateListRecord) => {
    ...
    const { data: rawVersions, error: versionError } = await client
      .from("rate_list_versions")
      .select(
        "id, rate_list_id, version_number, status, effective_from, effective_to, created_at, updated_at",
      )
      .in(
        "rate_list_id",
        applicableRateLists.map((rateList: RateListRecord) => rateList.id),
      )
      .eq("status", "ACTIVE");
    ...
    const { data: rawItems, error: itemError } = await client
      .from("rate_list_items")
      .select(
        "id, rate_list_version_id, product_id, minimum_quantity, unit_price, unit, created_at, updated_at",
      )
      .in(
        "rate_list_version_id",
        applicableVersions.map((version: RateListVersionRecord) => version.id),
      )
      .eq("product_id", context.product_id)
      .lte("minimum_quantity", context.quantity);
    if (itemError) throw itemError;

    const items = (rawItems ?? []) as RateListItemRecord[];
    const candidates = items.map((item: RateListItemRecord) => {
      const version = applicableVersions.find(
        (candidate: RateListVersionRecord) => candidate.id === item.rate_list_version_id,
      );
      if (!version) return null;

      const rateList = applicableRateLists.find(
        (candidate: RateListRecord) => candidate.id === version.rate_list_id,
      );
      ...
      (candidate): candidate is {
        item: RateListItemRecord;
        version: RateListVersionRecord;
        rateList: RateListRecord;
      } => candidate !== null,
    );
    ...
    candidates.sort((a, b) => {
      ...
      const effective =
        Date.parse(b.version.effective_from) - Date.parse(a.version.effective_from);
    ...
    return {
      rate_list_id: best.rateList.id,
      rate_list_version_id: best.version.id,
      rate_list_item_id: best.item.id,
      product_id: best.item.product_id,
      unit_price: best.item.unit_price,
      unit: best.item.unit,
      currency_code: best.rateList.currency_code,
      minimum_quantity: best.item.minimum_quantity,
      scope_type: best.rateList.scope_type,
      effective_from: best.version.effective_from,
    };
  }
