...
const inventoryPageSchema = pageSchema.extend({
  product_id: idSchema.optional(),
  warehouse_id: idSchema.optional(),
});
...
const warehouseSchema = z.strictObject({
  name: shortText,
...
type ErpServiceList<Entity extends { id: number }> = (
  page: z.output<typeof pageSchema>,
) => Promise<{ data: Entity[]; next_cursor: number | null }>;
...
function registerCrud<CreateInput, UpdateInput, Entity extends { id: number }>(
  ...
  router.get(path, authorize, async (request, response) => {
    response.status(200).json(await actions.list(pageSchema.parse(request.query)));
  });
  ...
  router.post(path, authorize, async (request, response) => {
    const record = await actions.create(createSchema.parse(request.body));
...
export function createErpRouter(
  ...
  registerCrud<BrandInput, Patch<BrandInput>, Brand>(
    router,
    "/brands",
    "Brand",
    authorize,
    brandSchema,
    atLeastOneField(brandSchema),
    {
      list: (page) => service.listBrands(page),
      get: (id) => service.getBrand(id),
      create: (input) => service.createBrand(input),
      update: (id, input) => service.updateBrand(id, input),
      delete: (id) => service.deleteBrand(id),
    },
  );

  registerCrud<CustomerInput, Patch<CustomerInput>, Customer>(
    router,
    "/customers",
    "Customer",
    authorize,
    partySchema,
    atLeastOneField(partySchema),
    {
      list: (page) => service.listCustomers(page),
      get: (id) => service.getCustomer(id),
      create: (input) => service.createCustomer(input),
      update: (id, input) => service.updateCustomer(id, input),
      delete: (id) => service.deleteCustomer(id),
    },
  );

  registerCrud<VendorInput, Patch<VendorInput>, Vendor>(
    router,
    "/vendors",
    "Vendor",
    authorize,
    ...
    atLeastOneField(partySchema),
    {
      list: (page) => service.listVendors(page),
      get: (id) => service.getVendor(id),
      create: (input) => service.createVendor(input),
      update: (id, input) => service.updateVendor(id, input),
      delete: (id) => service.deleteVendor(id),
    },
  );

  registerCrud<ProductInput, Patch<ProductInput>, Product>(
    router,
    "/products",
    "Product",
    authorize,
    productSchema,
    atLeastOneField(productSchema),
    {
      list: (page) => service.listProducts(page),
      get: (id) => service.getProduct(id),
      create: (input) => service.createProduct(input),
      update: (id, input) => service.updateProduct(id, input),
      delete: (id) => service.deleteProduct(id),
    },
  );

  registerCrud<WarehouseInput, Patch<WarehouseInput>, Warehouse>(
    router,
    "/warehouses",
    "Warehouse",
    authorize,
    warehouseSchema,
    atLeastOneField(warehouseSchema),
    {
      list: (page) => service.listWarehouses(page),
      get: (id) => service.getWarehouse(id),
      create: (input) => service.createWarehouse(input),
      update: (id, input) => service.updateWarehouse(id, input),
      delete: (id) => service.deleteWarehouse(id),
    },
  );

  router.get("/inventory", authorize, async (request, response) => {
    response.status(200).json(await service.listInventory(inventoryPageSchema.parse(request.query)));
  });

  router.get("/stock-movements", authorize, async (request, response) => {
    response
      .status(200)
      .json(await service.listStockMovements(inventoryPageSchema.parse(request.query)));
  });

  router.get("/purchases", authorize, async (request, response) => {
    response.status(200).json(await service.listPurchases(pageSchema.parse(request.query)));
  });
