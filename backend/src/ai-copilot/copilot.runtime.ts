...
export class CopilotRuntime {
  ...
  private async execute(plan: CopilotActionPlan, idempotencyKey: string): Promise<unknown> {
    ...
    const estimatePricing = new DefaultEstimatePricingService(this.pricing);
    ...
    if (plan.target === "invoice") {
      ...
      return this.salesTransaction.execute({ invoice, warehouse_id: plan.warehouseId, lines: transactionLines, idempotency_key: idempotencyKey });
    }
    ...
    if (plan.target === "supplier_bill") {
      ...
      const purchaseLines = await Promise.all(lines.map(async (line) => {
        ...
        if (unitCost === undefined) {
          const resolved = await this.pricing.resolvePrice({ product_id: line.productId, quantity: line.quantity, price_type: "PURCHASE", as_of: date, vendor_id: vendorId, rate_list_id: line.rateListId ?? null });
    ...
    if (plan.target === "inventory_adjustment") {
      ...
      const line = lines[0]!;
      const { data, error } = await client().rpc("record_inventory_adjustment", { p_product_id: line.productId, p_warehouse_id: plan.warehouseId, p_new_quantity: line.quantity, p_reason: plan.reason, p_principal_id: plan.userId, p_idempotency_key: idempotencyKey, p_request_fingerprint: copilotFingerprint(plan) });
      if (error) throw error;
      return data;
    }
