...
const required = [
  "index.html", "app.js", "auth.js", "copilot-api.js", "ai-experience.js",
  "offline-store.js", "offline-sync.js", "styles.css", "sw.js", "manifest.webmanifest",
];
for (const file of required) {
  const path = resolve(root, file);
  if (!existsSync(path)) throw new Error(`Missing production asset: ${file}`);
  const content = readFileSync(path, "utf8");
  if (!content.trim()) throw new Error(`Empty production asset: ${file}`);
}
const html = readFileSync(resolve(root, "index.html"), "utf8");
for (const marker of ["/styles.css", "/app.js", "/manifest.webmanifest"]) {
  if (!html.includes(marker)) throw new Error(`index.html missing production reference: ${marker}`);
}
const manifest = JSON.parse(readFileSync(resolve(root, "manifest.webmanifest"), "utf8"));
if (manifest.display !== "standalone") throw new Error("PWA manifest must use standalone display");
if (!Array.isArray(manifest.icons) || manifest.icons.length === 0) throw new Error("PWA manifest requires production icons");
