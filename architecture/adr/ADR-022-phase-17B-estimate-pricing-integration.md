...
## Decision
Estimates use the deterministic pricing engine to resolve unit prices for AI/OCR/voice/manual line candidates whenever a valid selected rate-list context or explicit rate-list/brand hint is available.

AI may extract item, quantity, unit, and brand/rate-list hints, but it never directly mutates financial pricing. Ambiguous or unresolved pricing remains explicit and requires user resolution. Manual override remains supported and auditable.
